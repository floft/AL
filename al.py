...
class AL:
        ...
        for event in in_data.rows_dict:
            ...
            if count >= self.conf.samplesize:
                ...
                for clf_name, clf in classifiers.items():
                    new_labels[clf_name] = clf.predict(xdata)[0]

                if count == self.conf.windowsize:
                    # We've just reached the first full window
                    # Write out all events in the window with this label:
                    for win_event in window_events:
                        self.write_event(out_data, win_event, new_labels, output_special_format)
                else:
                    # Only write out the most recent event:
                    self.write_event(out_data, window_events[-1], new_labels, output_special_format)
    ...
    def write_event(
            ...
            labels: OrderedDict[str, Union[str, int, float, None]],
            output_as_special: bool = False
    ):
        """
